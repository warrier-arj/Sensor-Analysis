		...
		while not rospy.is_shutdown():
			...
			if line == ' ':
				...
				if line.startswith('GPGGA'):
					...
					#Calling to_sec() function
					longit = float(sep[4])
					if (sep[5]=="W"):
						longit=longit*-1
					msg.Altitude = float(sep[9])
					ut = utm.from_latlon(lat_normal(sep[2]), lon_normal(str(longit)))		
					#Calling lat_normal() and lon_normal() function to convert
					# latitude and longitude to degrees
